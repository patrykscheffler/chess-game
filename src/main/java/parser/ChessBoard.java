...
public class ChessBoard {
	 ...
	 */
	public void loadFEN(String fen) throws IncorrectFENException {
		for(char keyVar: this.squares.keySet()) {
			for(int j=1; j<=8; j++) {
	 ...
	 */
	public String castle(String castling) {
		int line;
		if(this.currentMove == PieceColor.WHITE) {
			line = 1;
		} else {
		...
		if(castling.matches("^O-O\\+?$")) {
            ...
            result = "e"+line+"g"+line+" "+"h"+line+"f"+line;
		} else {
			this.makeMove('e', line, 'c', line, false);
			this.makeMove('a', line, 'd', line, false);
            result = "e"+line+"c"+line+" "+"a"+line+"d"+line;
		}
		...
		this.enPassant = "-";
		this.halfMoves++;
		if(this.currentMove == PieceColor.BLACK) {
			this.fullMoves++;
		}
	...
	public void moveHandler(String piece, char fromX, int fromY, char toX, int toY, boolean capture, boolean promotion, PieceType promoteTo) {
		// Make piece move
		...
		if(piece.equals("pawn")) {
			...
			if(Math.abs(toY - fromY)==2 && ((toX!='a' && this.squares.get(this.letters[this.letter.get(toX)-1])[toY].piece!=null
					...
					&& this.squares.get(this.letters[this.letter.get(toX)-1])[toY].piece.type == PieceType.PAWN) ||
			...
			if (promotion) {
				this.squares.get(toX)[toY].piece.type = promoteTo;
			}
		} else {
			this.enPassant = "-";
